...
double Matriz::calcularDeterminante(){
    ...
    double determinante;
    double aux;
	if(this->n_columnas==2)
		return (this->matriz[0][0]*this->matriz[1][1])-(this->matriz[0][1]*this->matriz[1][0]);
    for (int i=0;i<this->n_filas;i++) {
        for (int j=0;j<this->n_filas-1;j++) {
        	Matriz m(j,j);
            for (int k=0;k<this->n_filas-1;k++) {
                if (k<i)
                    m.matriz[j][k]=this->matriz[j+1][k];
                if(k>=i)
                    m.matriz[j][k]=this->matriz[j+1][k+1];
            }
            aux+=factor2*m.matriz[0][j]*m.calcularDeterminante();
            factor2*=-1;
        }
        determinante+= factor*this->matriz[0][i]*aux;
        factor*=-1;
    }
    return determinante;	
}
